...
A render is the trigger because a render is how an edit reaches the webview (debounced at 300 ms). Two other render paths exist and neither can reach live query state — the hidden→visible self-heal fires only after VSCode has destroyed the webview's context, and re-invoking *Kul: Show Preview* is an explicit gesture — so the rule is **every render that can reach live query state is either an edit or an explicit command**. The accepted cost is that a one-character typo fix costs the reader their selection, their paint and their filter: **no query artefact outlives the source it was computed from**. Refetching per render was measured as affordable (≈13 ms) and rejected on that principle, not on cost. `QuerySurface.endQueryMode()`, composed of `clearSelection()` and `clearFilter()`.
