...
The top-level value `kul_render::compute` and `kul_render::transform` emit — the canonical-UI-pattern projection of a checked Kul project. Either a success payload (`components`, `edges`, plus the `schema` / `kul` discriminators) or a failure payload (the same diagnostic list the input [`ExportEnvelope`](#exportenvelope) carried). Stage 2 of the renderer pipeline (Stage 1 is [`ExportEnvelope`](#exportenvelope); Stage 3 is downstream layout). Defined at `crates/kul-render/src/shape.rs`; the principles it realizes (P1–P16) live in [`docs/canonical-ui-pattern.md`](./docs/canonical-ui-pattern.md); crate-boundary rationale in [ADR-0016](./docs/adr/0016-kul-render-crate-boundary.md); schema-versioning policy in [ADR-0017](./docs/adr/0017-render-shape-schema-and-versioning.md).
