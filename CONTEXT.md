...
The state of *reading* a family rather than authoring it: an [entity selection](#entity-selection), a painted [kin set](#kin-paint), an active [filter](#filter-bar), or any combination — everything the preview holds that was computed from a project snapshot. It is not a toggle and there is no button for it; the reader enters it by clicking, and it is defined by what is on screen.

**An edit ends it** ([ADR-0035](./docs/adr/0035-detail-surface-one-selection-one-batched-lookup.md), composed in [ADR-0046](./docs/adr/0046-the-mode-boundarys-render-paths-and-what-the-retired-documents-leave-behind.md)). One render clears the selection, its panel, the kin paint, the [hover lens](#hover-lens) and the filter; the tree returns to plain and [editor sync](#sync-suspension) resumes on its own, because both suspension reasons lift together and the held highlight replays. **Esc is the same exit** — two ways out, one behaviour, one function. What survives is what belongs to the reader rather than to the source: the [locale](#locale-toggle) and the [Explore-kin list](#explore-kin-list)'s open state.

A render is the trigger because a render is how an edit reaches the webview (debounced at 300 ms). Three other render paths exist, and the rule they satisfy is **every render that can reach live query state is either an edit or an explicit command**: the panel's first render and the hidden→visible self-heal cannot reach live query state at all (the webview has only just mounted, VSCode having destroyed its context), while re-invoking *Kul: Show Preview* on a live panel **can** — and is allowed to, because the reader asked for it by name. The accepted cost is that a one-character typo fix costs the reader their selection, their paint and their filter: **no query artefact outlives the source it was computed from**. Refetching per render was measured as affordable (≈13 ms) and rejected on that principle, not on cost. `QuerySurface.endQueryMode()`, composed of `clearSelection()` and `clearFilter()`.
