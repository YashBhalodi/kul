...
//!   `kula-core::tests::export::failure_envelope_*` so any drift between
//!   the CLI export envelope and the WASM `check` projection surfaces
//!   immediately.
//! - **Clean-corpus sweep** — every `examples/*.kula` must produce an
//!   empty `diagnostics` array. The corpus-contract guard mirrors the
//!   pattern in `kula-core::tests::export` and `format.rs`: dropping a
//!   new example forces a snapshot review here.
//!
//! Broken inputs live as inline strings; the `examples/*.kula` corpus
...
fn check_json(source: &str) -> String {
    let envelope = kula_wasm::check(source);
...
macro_rules! clean_example {
    ($name:ident, $stem:literal) => {
        ...
        fn $name() {
            ...
            let envelope = kula_wasm::check(&read(&path));
            assert!(
