...
///
/// The token layer mirrors the preview's two tiers (ADR-0036): tier 1 is
/// the theme contract — colour roles, the diagram hues, the diagram font
/// — and tier 2 is the per-site semantic layer aliasing onto it. The
/// subset rule is what "structural subset" now means concretely: the
/// chrome half of both tiers (raised-surface interaction states, the
/// spacing / radius / motion scales, the reserved query paint) is skipped,
/// because none of it has an application site in a bare SVG.
///
/// `--kul-ghost-stroke` keeps a literal rather than aliasing `--kul-muted`:
/// this theme has always drawn the ghost border a shade lighter than the
/// ghost label, and ADR-0036 re-expresses values without re-choosing them.
/// It is the per-site override ADR-0016 kept open, used as intended.
const SELF_CONTAINED_STYLE: &str = r#"<style>
svg {
  --kul-surface: #ffffff;
  --kul-raised: #f7f9fa;
  --kul-on-surface: #1a1a1a;
  --kul-muted: #607d8b;
  --kul-border: #cfd8dc;
  --kul-font-diagram: system-ui, -apple-system, "Segoe UI", Roboto, sans-serif;
  --kul-hue-male: #1565c0;
  --kul-hue-female: #c2185b;
  --kul-hue-other: #f9a825;
  --kul-hue-birth: #2e7d32;
  --kul-hue-adoption: #ef6c00;
  --kul-hue-marriage: #6a1b9a;
  --kul-hue-past: #eceff1;
  --kul-preview-bg: var(--kul-surface);
  --kul-font-family: var(--kul-font-diagram);
  --kul-card-fill: var(--kul-surface);
  --kul-card-stroke: var(--kul-on-surface);
  --kul-card-stroke-male: var(--kul-hue-male);
  --kul-card-stroke-female: var(--kul-hue-female);
  --kul-card-stroke-other: var(--kul-hue-other);
  --kul-ghost-fill: var(--kul-hue-past);
  --kul-ghost-stroke: #90a4ae;
  --kul-ghost-stroke-male: var(--kul-hue-male);
  --kul-ghost-stroke-female: var(--kul-hue-female);
  --kul-ghost-stroke-other: var(--kul-hue-other);
  --kul-label-fill: var(--kul-on-surface);
  --kul-ghost-label-fill: var(--kul-muted);
  --kul-edge-stroke: var(--kul-hue-birth);
  --kul-adoption-edge-stroke: var(--kul-hue-adoption);
  --kul-marriage-edge-stroke: var(--kul-hue-marriage);
  ...
  --kul-legend-panel-bg: var(--kul-raised);
  --kul-legend-panel-border: var(--kul-border);
