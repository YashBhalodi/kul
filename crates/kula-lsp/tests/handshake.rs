//! Integration test: spawn `kula-lsp` as a subprocess, complete the LSP
...
fn binary_path() -> std::path::PathBuf {
    ...
    // produces.
    let path = env!("CARGO_BIN_EXE_kula-lsp");
...
impl Handle {
    fn spawn() -> Self {
        ...
        cmd.env("RUST_LOG", "kula_lsp=debug")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
...
fn did_open_notification(uri: &str, text: &str) -> String {
    ...
    format!(
        r#"{{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{{"textDocument":{{"uri":"{uri}","languageId":"kula","version":1,"text":{escaped_text}}}}}}}"#
    )
}
...
fn handshake_and_did_open() {
    ...
    assert!(
        init_response.contains("kula-lsp"),
    ...
    write_message(
        &mut handle.stdin,
        &did_open_notification("file:///alice.kula", "kula 1\nperson alice\n"),
    );
