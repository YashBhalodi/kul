...
//! `kula_core::check` produces directly.
//!
...
#[test]
fn publish_diagnostics_match_kula_core() {
    ...
    let did_open = format!(
        r#"{{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{{"textDocument":{{"uri":"file:///fixture.kula","languageId":"kula","version":1,"text":{escaped}}}}}}}"#
    );
    ...
    assert_eq!(params["uri"].as_str().expect("uri"), "file:///fixture.kula");

    let lsp_diags = params["diagnostics"].as_array().expect("diagnostics array");
    let core_diags = kula_core::check(FIXTURE).diagnostics;
    let line_index = LineIndex::new(FIXTURE);

    assert_eq!(
        lsp_diags.len(),
        core_diags.len(),
        "lsp diagnostic count diverged from kula_core::check"
    );
...
fn close_clears_diagnostics() {
    ...
    let did_open = r#"{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{"textDocument":{"uri":"file:///c.kula","languageId":"kula","version":1,"text":"kula 1\nperson a\n"}}}"#;
    write_message(&mut handle.stdin, did_open);
    ...
    let did_close = r#"{"jsonrpc":"2.0","method":"textDocument/didClose","params":{"textDocument":{"uri":"file:///c.kula"}}}"#;
