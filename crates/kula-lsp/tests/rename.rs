...
fn open_fixture(handle: &mut Handle) {
    ...
    let did_open = format!(
        r#"{{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{{"textDocument":{{"uri":"file:///rn.kula","languageId":"kula","version":1,"text":{escaped}}}}}}}"#
    );
...
fn rename_returns_workspace_edit_covering_decl_and_refs() {
    ...
    let we = &resp["result"];
    assert!(!we.is_null());
    let edits = we["changes"]["file:///rn.kula"]
        .as_array()
