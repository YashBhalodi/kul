...
fn open_fixture(handle: &mut Handle) {
    ...
    let did_open = format!(
        r#"{{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{{"textDocument":{{"uri":"file:///def.kula","languageId":"kula","version":1,"text":{escaped}}}}}}}"#
    );
...
fn definition_at(handle: &mut Handle, id: i64, line: u32, character: u32) -> Value {
    let req = json!({
        ...
        "params": {
            "textDocument": { "uri": "file:///def.kula" },
...
fn definition_on_spouse_ref_jumps_to_person_decl() {
    ...
    assert_eq!(result["uri"].as_str(), Some("file:///def.kula"));
