//! Integration test: spawn `kula-lsp`, complete the handshake, open a
//! document, send a `kula/export` custom request, and verify the response
//! envelope.
//!
...
fn handshake(handle: &mut Handle) {
    ...
    // Verify the experimental capability advertises kulaExport.
    let experimental = &init["result"]["capabilities"]["experimental"];
    assert_eq!(experimental["kulaExport"]["formats"][0], "json");
    assert_eq!(experimental["kulaExport"]["formats"][1], "cytoscape");
    write_message(
        &mut handle.stdin,
...
fn send_export(
    ...
    let req = format!(
        r#"{{"jsonrpc":"2.0","id":{id},"method":"kula/export","params":{{"uri":"{uri}","format":"{format}","withPositions":{with_positions}}}}}"#
    );
...
fn export_clean_document_returns_success_envelope() {
    ...
    let response = send_export(&mut handle, 100, "file:///clean.kula", "json", false);
...
fn export_dirty_document_returns_failure_envelope() {
    ...
    let response = send_export(&mut handle, 200, "file:///dirty.kula", "json", false);
    let envelope = &response["result"];
...
fn export_cytoscape_format_returns_nodes_and_edges() {
    ...
    let response = send_export(&mut handle, 300, "file:///cy.kula", "cytoscape", false);
    let envelope = &response["result"];
    assert_eq!(envelope["ok"], true);
...
fn export_unknown_document_returns_invalid_params_error() {
    let mut handle = Handle::spawn();
    handshake(&mut handle);
    let response = send_export(&mut handle, 400, "file:///never-opened.kula", "json", false);
