...
//! maps a `KULA-Rxx` code to a provider function that, given the
...
fn registry() -> HashMap<&'static str, ProviderFn> {
    ...
    m.insert("KULA-R03", r03_required_fields);
    m.insert("KULA-R05", r05_end_consistency);
    m
}
...
fn default_end_reason() -> kula_core::ast::EndReasonValue {
    kula_core::ast::EndReasonValue {
        value: kula_core::ast::EndReason::Divorce,
...
mod tests {
    ...
    fn unrelated_diagnostic_codes_are_ignored() {
        // KULA-R04 (self-marriage) doesn't have a registered fix.
        let src = "person alice name:\"A\" gender:female\n\
    ...
    fn end_reason_fix_resolves_diagnostic_after_apply() {
        ...
        let result = kula_core::check(&fixed);
        assert!(
            !result
                .diagnostics
                .iter()
                .any(|d| d.code.starts_with("KULA-R05")),
    ...
    fn missing_gender_fix_resolves_diagnostic_after_apply() {
        ...
        assert!(
            !result
                .diagnostics
                .iter()
                .any(|d| d.code == "KULA-R03" && d.message.contains("`gender:`")),
            "diagnostics after fix: {:?}",
            result.diagnostics,
        );
    }
    ...
    fn range_filter_excludes_unrelated_lines() {
        ...
        let result = kula_core::check(src);
        let resolved = result.resolved();
