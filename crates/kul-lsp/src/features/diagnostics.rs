...
/// Translate a slice of `kul-core` diagnostics into LSP diagnostics.
///
...
/// Translate a single `kul-core` diagnostic. The single point where
...
mod tests {
    ...
    fn passes_code_message_source() {
        let core = CoreDiagnostic::error("KUL-R03", "missing name", ByteSpan::new(0, 5));
        let lsp = &to_lsp(&url(), std::slice::from_ref(&core), &idx("hello"))[0];
        assert_eq!(lsp.code, Some(NumberOrString::String("KUL-R03".into())));
        assert_eq!(lsp.message, "missing name");
        assert_eq!(lsp.source.as_deref(), Some("kul"));
    ...
    fn range_uses_utf16_for_multibyte() {
        let source = "a🎉b";
        let core = CoreDiagnostic::error("KUL-R01", "duplicate", ByteSpan::new(1, 5));
    ...
    fn related_info_carries_doc_uri() {
        let core = CoreDiagnostic::error("KUL-R01", "duplicate", ByteSpan::new(0, 5))
    ...
    fn no_related_info_field_when_empty() {
        let core = CoreDiagnostic::error("KUL-R03", "missing", ByteSpan::new(0, 1));
