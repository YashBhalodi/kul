//! Kul language server.
//!
//! Thin adapter over `kul-core`: exposes lex/parse/resolve/validate as LSP
...
//! `kul-core`. The async layer here is purely for LSP-protocol concurrency.
//!
//! # Modules
//!
...
/// protocol channel). Set `RUST_LOG=kul_lsp=debug` for verbose logs.
pub async fn run() {
    init_tracing();
