...
//!
//! Holds the source text, the cached `kul_core::CheckResult`, and a
...
mod tests {
    ...
    async fn open_caches_source_and_check() {
        ...
        docs.open(url("file:///a.kul"), "kul 1\nperson alice\n".into())
            .await;
        let stored_source = docs
            .with(&url("file:///a.kul"), |d| d.source.clone())
            .await
            .unwrap();
        assert_eq!(&*stored_source, "kul 1\nperson alice\n");
    }

    #[tokio::test]
    async fn update_replaces_check() {
        let docs = Documents::new();
        docs.open(url("file:///a.kul"), String::new()).await;
        docs.update(
            url("file:///a.kul"),
        ...
        .await;
        let diag_count = docs
            .with(&url("file:///a.kul"), |d| d.check.diagnostics.len())
            .await
            .unwrap();
        // Cached check matches a fresh check of the same source.
        let fresh = kul_core::check("kul 1\nperson alice name:\"A\" gender:female\n");
