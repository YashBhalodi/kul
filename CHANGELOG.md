...
- **Clicking the preview now selects an entity, and a details panel says what it is.** Person cards, marriage edges and adoption edges are all selectable; clicking another entity moves the one selection, and Esc or a click on empty canvas clears it. The selection populates a floating panel — one variant per entity kind — filled entirely by one batched engine call, so what the panel says is what the contract says rather than what the picture happens to carry. Absent fields are omitted rather than shown as "not recorded", and each marriage gets its own line with its span and end reason, which is what explains a person appearing on more than one card. Every person named in the panel is clickable: clicking moves the selection *and* pans the canvas to centre that person's card clear of the panel, so a panel-driven walk never parks someone out of sight. Editor⇄preview sync pauses for as long as a selection exists (hint, bottom right) and resumes on Esc, so the two meanings of "highlighted" never paint at once. See ADR-0035, ADR-0037 and ADR-0042 (#300).
- **Removed: the hover tooltip.** Hovering a card or edge no longer opens a detail tooltip. Reading a person now costs a click, and the details panel is the one detail surface — the tooltip's replacement is deliberately not zero-commitment, because hover is becoming the relationship lens. This removes a working feature; it is the price of one surface per question, and of ending the tooltip's habit of reading its content off the rendered SVG (#300).
- **Removed: click-to-source on the tree.** Clicking a card or a marriage bar no longer jumps the editor to that statement. Revealing is now an explicit **Reveal in editor** control on the panel header — which also means it works for marriages, not just persons, and that walking a family no longer drags the editor's scroll position along behind it. An adoption has no id of its own, so its panel reveals the adopted child (#300).
