...
- **New `renderSvg(files, manifest)` operation** — runs the full canonical-visual pipeline (`kul_render::compute` → `kul_layout::layout` → `kul_svg::render`) inside the wasm bridge and returns a `RenderEnvelope` (`{ ok: true, svg: string }` on success, `{ ok: false, diagnostics: ExportedDiagnostic[] }` on failure). JSON shape is bit-identical to the LSP's `kul/render` response (enforced by a cross-surface snapshot test in `crates/kul-wasm/tests/render_svg.rs`), so JS-ecosystem consumers (web playground, future web app) can produce the same canonical SVG the VSCode preview produces without shelling out to the LSP. Same input shape as `check` / `exportGraph` (files array + manifest); no options parameter in v1 — only `LayoutConfig::default()` and `ThemeConfig::default()` are constructed. A native-callable `render_svg_with(&[InputFile], &Manifest)` mirror lives alongside the `#[wasm_bindgen]` entry, matching the `export_with` precedent so non-wasm tests drive the operation without `JsValue` round-trips. Failure-arm `ExportedDiagnostic` shape matches what `exportGraph`'s failure envelope produces, so consumers share their diagnostic-rendering code across both operations. The committed `crates/kul-wasm/types/kul_wasm.d.ts` snapshot regenerates with `RenderEnvelope` / `RenderSuccess` / `RenderFailure` / `renderSvg` per [ADR-0012](./docs/adr/0012-tsify-derived-types-committed-and-diffed.md); the TypeScript consumer compile-test exercises structural narrowing on the new envelope. Amends [ADR-0011](./docs/adr/0011-wasm-surface-three-shapes-no-wrappers.md) in place — the surface is now operation-specific shapes plus this fourth operation, no convenience layer (#126).
