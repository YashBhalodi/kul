...
- **Theming is a CSS concern.** The VSCode preview maps the class vocabulary to theme values across two stylesheets — `packages/preview/src/preview-themes.css` (the per-theme token definitions) and `packages/preview/src/preview.css` (the application rules that consume them); structured as a token layer (see the amendment below). A web app maps the same classes to its brand palette. Adding a theme is a stylesheet change, not a Rust release.
...
## Amendment (2026-06-09) — preview chrome lives in `@kullang/preview`, surface hosts adapt to it

The structural/chrome line still draws between Rust and the surface; what changes is *where the chrome implementation lives*. The webview chrome — HTML shell, bootstrap, tooltip, legend, pan/zoom controls, error popover, ghost-badge injection, selection-sync highlighting, application CSS, `--kul-*` theme tokens — moves from inside the VSCode extension directory into a workspace npm package `@kullang/preview` at `packages/preview/`. The VSCode extension becomes a thin host that imports `previewHtml` to set the webview HTML, ships the bundled webview entry script, and consumes the package's `HostAdapter` interface — outbound `onRevealRequest` from the chrome, inbound `render` / `showErrors` / `highlightEntity` from the LSP responses. A future webapp implements its own `HostAdapter` against the same interface and reuses the chrome unchanged.

Two consequences worth naming:

- **The structural/chrome line is unchanged.** Rust still owns structural SVG; surfaces still own chrome and interactivity. The new fact is that *the chrome implementation* is now a single workspace package shared across surfaces, not duplicated per host. The class + `data-*` vocabulary the package consumes remains the kul-svg contract.
