...
- **Theming is a CSS concern.** The VSCode preview maps the class vocabulary to theme values in a stylesheet (`editor/vscode/media/preview.css`) — structured as a token layer (see the amendment below); a web app maps the same classes to its brand palette. Adding a theme is a stylesheet change, not a Rust release.
