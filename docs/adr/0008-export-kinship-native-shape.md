...
- **"Reify canonical-UI-pattern derivations (host, current intimacy, ghosts, recursive nesting) as export fields."** The audit recorded in #117 verified that the existing kinship-native shape carries every piece of data the canonical UI pattern ([`docs/canonical-ui-pattern.md`](../canonical-ui-pattern.md)) needs. Layout derivations are the renderer's responsibility, not the export's. Adding a `host: true` flag to spouses, a `currentIntimacy` pointer on persons, or pre-computed ghost markers would violate this ADR's foundational principle and freeze layout semantics into the schema.
