...
The forcing input is the **kinship-term inventory** ([#278](https://github.com/YashBhalodi/kul/issues/278)) — every lexicalized Gujarati and English kinship term expressed against the descriptor's dimensions. Four of its findings are load-bearing here, and the inventory was transient (it graduated into `en`/`gu` term data when the epic shipped and was then deleted, per [ADR-0046](./0046-the-mode-boundarys-render-paths-and-what-the-retired-documents-leave-behind.md)), so they are lifted into this ADR:
...
- **The term inventory can now graduate.** Its findings 1, 2, 3, 5, 6 and 7 are recorded above; when the epic ships the `en` / `gu` packs, the inventory is deleted per its stated lifecycle. (It did, and [ADR-0046](./0046-the-mode-boundarys-render-paths-and-what-the-retired-documents-leave-behind.md)'s appendix records the register variants the packs deliberately did not take.)
