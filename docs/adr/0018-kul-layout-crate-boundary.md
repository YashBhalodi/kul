...
- **`kul_layout::adapter`** — wraps Walker's for kul's pattern: marriage bar between adjacent spouses, ghost slots at the host's birth-family position per P8, generation rows from generation indices, orthogonal right-angle edge routing (for both `InTree` and `CrossTree` edges — see below), future recursive P6 nesting. Hides Walker's complexity from `kul_layout::layout`.
...
`PositionedEdge::routing` is an extensible discriminator with `InTree` and `CrossTree` variants. **Both variants emit the same orthogonal right-angle polyline geometry and the same attachment points** — bar bottom-midpoint, horizontal bus at `card_top - config.bus_drop`, child card top-midpoint — so the entire diagram follows one consistent edge-routing pattern (P1). The discriminator exists to give downstream consumers a future re-theming hook (the emitted CSS classes `kul-edge--in-tree` / `kul-edge--cross-tree` differ); the layout layer treats the two identically. The adapter routes an edge as `CrossTree` when both endpoints sit in the laid-out tree but the child is *not* a structural descendant of that marriage's bar — the canonical exerciser is the cousin-marriage case (P11) in `examples/11-cousin-marriage/`, where the joining cousin's birth-edge connects back to a sibling marriage already in the rendering context.
