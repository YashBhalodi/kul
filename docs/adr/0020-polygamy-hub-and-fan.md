...
- **Children of each marriage at row R+2**, centred on the marriage-edge midpoint `children_center_i = (hub_cx + cospouse_cx_i) / 2`. Each marriage's children forest keeps its full Walker layout (nested birth families, deeper generations, even recursive polygamy); the fan pins only the forest's block centre to the midpoint, so half-siblings render in distinct columns, one per marriage. This is the **children hang from the marriage-edge midpoint** principle applied to the fan.
...
**Integration with Walker.** The hub is a single Walker leaf whose width reserves the full wing-to-wing extent (symmetric about the hub centre), so a fan packs cleanly against sibling components and nests inside a larger tree without overlap. Each marriage's children forest is built through the usual `build_person` recursion (so nesting, deeper generations, and recursive polygamy keep their tidy-tree treatment) and measured for `CW_i` by a per-forest Walker pass; the forests are attached as the hub leaf's Walker children so the global Walker reserves their contour, then rigidly translated in the projection pass so each block centre lands on its prescribed `children_center_i`. Because the prescribed spread is always ≥ the forests' natural spacing and the reserved hub width covers the wider wings, the override never pushes a forest outside the reserved footprint. The hub seats its children two rows below itself, so its descendant-pull term in the visual-row cascade ([ADR-0018](./0018-canonical-layout-algorithm.md)) reads `min(child.visual_row) - 2.0`.
...
A marriage renders as a thick `EdgeKind::Marriage` edge for both monogamy and polygamy — there is no separate bar primitive. A monogamy marriage is a thick horizontal edge spanning the inter-card gap between the two adjacent spouse cards at their vertical mid-height (`[(left_card_right_edge, mid_y), (right_card_left_edge, mid_y)]`), and the couple's children drop from its midpoint. An ended (divorced) marriage carries an `ended` flag through to the `kul-edge--ended` class (translucent). `kul-svg` picks the `kul-edge--marriage` modifier in its existing `write_edge`, sharing all routing CSS with birth and adoption edges; the only visual delta is stroke weight (markedly heavier than the 1.5px birth / adoption edges), so a marriage reads as distinct while staying inside one coherent edge vocabulary.

The `MarriageBar` **data** type in `kul-render` is unchanged — only the rendered primitive is an edge rather than a block. `RenderShape` does not change ([ADR-0017](./0017-render-shape-schema-and-versioning.md) already gives a polygamy hub the right shape: one canonical `PersonCard` carrying N bars), so `RENDER_SCHEMA_VERSION` stays at `2`. In `PositionedShape`, `EdgeKind` carries a `Marriage` variant alongside `Birth` and `Adoption`, and `PositionedEdge` carries an `ended: bool` (default `false`; polygamy marriages are always un-ended per R14, and birth / adoption edges never set it). There is no `PositionedBar` type and no `bars` field.
...
- **No bars at all.** Every marriage — monogamy or polygamy — is a `PositionedEdge { kind: Marriage, .. }`. The `<rect class="kul-bar">` emission and the `kul-bar--ended` class are gone, replaced by `kul-edge--marriage` and `kul-edge--ended`.
