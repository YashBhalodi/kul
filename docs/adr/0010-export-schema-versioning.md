...
- **"Embed the schema version in the URL of a published JSON-Schema document."** Premature. We do not publish a JSON-Schema document for the envelope yet, and may never need to — the spec section ([§15](../../spec/16-export-schema.md)) is the contract. If a JSON-Schema document is later wanted, it can carry the same integer in its `$id`.
