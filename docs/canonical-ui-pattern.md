...
A person may have multiple concurrent un-ended marriages (polygamy / polyamory) — a **polygamy hub**. If the person has ≥2 un-ended marriages, their canonical card sits at the **fan hub position**: the hub card occupies its own row at row R alone; each co-spouse appears canonically adjacent on the fan sub-row (`R + fan_drop_fraction` per [ADR-0027](./adr/0027-fan-primitive-for-polygamy-hubs.md)) at their own marriage's bar, with the bar abutting the co-spouse card on the side facing the hub's vertical axis. A fan connector (trunk + horizontal branch + per-bar drops) links the hub to each bar in declaration order. No ghost is emitted for any current intimacy — the ghost vocabulary (P16) is reserved for *past* (ended / superseded) intimacies.
...
*Worked example (`examples/04`):* Devraj hosts Meera (`m_devraj_meera`, 1990) and Alice (`m_devraj_alice`, 1992), both ongoing. Devraj is the polygamy hub (two un-ended marriages); per ADR-0027 his canonical card renders alone at the fan hub position on row 0, with Meera and Alice as canonical co-spouses on the fan sub-row (row 0.5) below. Each marriage bar sits adjacent to its co-spouse on the side facing the hub's vertical axis (`m_devraj_meera` on Meera's right; `m_devraj_alice` on Alice's left). A fan connector — trunk from Devraj's bottom-midpoint, horizontal branch above the bar row, drops onto each bar top — links the hub to both bars. Neither marriage emits a past-marriage ghost (both are current). Priya hangs from `m_devraj_alice`'s bar specifically per P9, in her own column below that bar.

*Worked example (`examples/12`):* Same polygamy hub as example 04 (Devraj concurrently married to Meera and Alice, Priya the bio child of `m_devraj_alice`), but Devraj himself is now a canonical child of Ramesh + Sita's marriage at row 1. The fan still applies: Devraj's card sits alone on row 1; Meera and Alice render as co-spouses on the fan sub-row (row 1.5) below him; the fan connector links them; Priya hangs from `m_devraj_alice`'s bar in her own column on row 2.5. Ramesh + Sita's classical hub-and-flanks cluster sits on row 0 (monogamy, unchanged), with Devraj's birth edge attaching to `m_ramesh_sita`'s bar and entering Devraj's hub card from above.

*Worked example (`examples/15`):* N=3 polygamy. Devraj hosts Meera, Alice, and Diana, all ongoing. Devraj's canonical card sits alone at the fan hub position on row 0; the three co-spouses render on the fan sub-row in declaration order — Meera at the outer-left column, Alice in the middle column, Diana at the outer-right column. Each marriage bar abuts its co-spouse on the inner side: `m_devraj_meera` on Meera's right; `m_devraj_alice` on Alice's left; `m_devraj_diana` on Diana's left (the consistent left-bar treatment for middle and rightmost spouses per ADR-0027). The fan connector's branch spans all three bar centres. Each marriage has one child — Asha, Priya, Rohan — hanging in their own column below the corresponding bar; the three half-siblings render in distinct sub-trees per P9.
...
For polygamy hubs (P8 fan): each marriage's children hang from their own marriage bar's drop, not from the hub. Half-siblings render in distinct sub-trees, one per bar — e.g. in example 04, Priya hangs from `m_devraj_alice`'s bar only; in example 15 (N=3), Asha hangs from `m_devraj_meera`'s bar, Priya from `m_devraj_alice`'s, and Rohan from `m_devraj_diana`'s, each in their own column below their bar.
...
| **Fan connector** | Trunk + horizontal branch + per-bar drops linking a polygamy hub to each of its concurrent marriage bars (P8 fan, [ADR-0027](./adr/0027-fan-primitive-for-polygamy-hubs.md)). Thicker stroke than birth / adoption edges; unified visual weight with the marriage bar so the fan reads as one continuous "hub manifold" rather than a stack of independent edges. Structures the hub → bars → co-spouses path for polygamy at any N ≥ 2. |
