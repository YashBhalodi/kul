...
- **Visual baseline: classical descendency family tree.** Generations as horizontal rows (older above, younger below); spouses adjacent with a thick marriage edge between them; children connected below the midpoint of that marriage edge. This is the only kinship visual everyone already knows; the pattern's job is to extend it honestly for the cases the classical tree doesn't natively handle.
...
Each person renders as exactly one *canonical* card, which may host one or more marriages (P8). Visual duplicates (ghosts) exist only under the narrow conditions specified in P8 and P16.
...
A person may have multiple concurrent un-ended marriages (polygamy / polyamory) — a **polygamy hub**. If the person has ≥2 un-ended marriages, their canonical card sits at the **fan hub position** ([ADR-0027](./adr/0027-fan-primitive-for-polygamy-hubs.md)): the hub card occupies its own row at row R alone; each co-spouse appears canonically on row R+1 below, one per marriage in declaration order; and each marriage's children sit on row R+2 **centred at the midpoint of that marriage's edge**. The governing invariant is

> `children_center_i = (hub_cx + cospouse_cx_i) / 2`

i.e. each co-spouse is the mirror of the hub across that marriage's children-centre. Spouses therefore splay out toward the **wings** while every marriage's children gather toward the **centre**, directly under the midpoint of that marriage's thick **marriage edge**. Each marriage edge is one routed `<path>` from the hub card's bottom edge to the co-spouse's top edge, using the same orthogonal right-angle routing as a birth edge but with a heavier stroke; the fan emerges from N marriage edges fanning out of the single hub-bottom point. The marriage edge's horizontal segment runs at a bus just below the hub, and its midpoint is the children-centre — so that marriage's child birth/adoption edges originate from the midpoint of its marriage edge and fan down to the children. The marriage edge is the same unified connector monogamy uses (a thick stroke), routed hub→spouse here instead of horizontally between adjacent spouses. A **childless** co-spouse keeps the same wing/mirror placement with no children block; its marriage edge lands on its top-centre. No ghost is emitted for any current intimacy — the ghost vocabulary (P16) is reserved for *past* (ended / superseded) intimacies.

*Odd-N middle nudge:* with an odd number of marriages the middle co-spouse would naturally land on the hub column, where its child-drop would run straight down through the co-spouse's card. The layout nudges that middle co-spouse off the hub column so its children-centre clears the card by the same `gap/2` margin as every wing co-spouse, and the outer co-spouses splay wider in exchange to keep the hub centred (see ADR-0027). The child-at-midpoint invariant is unchanged — every child still sits at the midpoint of its marriage's edge.
...
*Worked example (`examples/04`):* Devraj hosts Meera (`m_devraj_meera`, 1990) and Alice (`m_devraj_alice`, 1992), both ongoing. Devraj is the polygamy hub; per ADR-0027 his canonical card renders alone at the fan hub position on row 0 (centre x = 296 with default metrics), with Meera and Alice as canonical co-spouses on row 1. Both spouses splay to the wings — Meera at 104, Alice at 488, each mirrored ±192 about the hub centre. Meera is childless: her marriage edge lands on her top-centre. Alice bears Priya (`m_devraj_alice`): Priya sits on row 2 at the Devraj–Alice midpoint, `(296 + 488)/2 = 392`, which is exactly the midpoint of Alice's marriage-edge horizontal segment. Priya's birth edge originates at that midpoint `(392, 156)` and drops straight to her card; the drop at x = 392 clears Alice's card (left edge 408) by 16px. Two thick marriage edges fan out of Devraj's single bottom-midpoint, only heavier than a birth edge. No marriage bar is rendered; neither marriage emits a past-marriage ghost (both current).

*Worked example (`examples/12`):* Same polygamy hub as example 04 (Devraj concurrently married to Meera and Alice, Priya the bio child of `m_devraj_alice`), but Devraj is now a canonical child of Ramesh + Sita's marriage. The fan still applies, shifted down one generation: Ramesh + Sita's classical hub-and-flanks cluster sits on row 0 (monogamy, unchanged — its thick marriage edge spans the gap between the two adjacent spouse cards), Devraj's hub card sits alone on row 1 centred directly under the `m_ramesh_sita` marriage edge (his birth edge enters his hub card from above), Meera and Alice splay to the wings on row 2, and Priya sits on row 3 at the Devraj–Alice midpoint. The whole fan nests as a single wide cluster under Ramesh + Sita without overlapping anything; the wing co-spouses are reserved in the hub's footprint so the components don't collide.

*Worked example (`examples/15`):* N=3 polygamy. Devraj hosts Meera, Alice, and Diana, all ongoing, one child each (Asha, Priya, Rohan). Devraj's hub card sits alone on row 0 (centre 680). The middle marriage (Alice) would land on the hub column, so its children-centre is nudged off to `hub + 96 = 776` and the outer two marriages splay wider to keep the hub centred. The three co-spouses splay across row 1 — Meera at the outer-left wing (104, `hub - 576`), Alice nudged just right of the hub (872, `hub + 192`), Diana at the outer-right wing (1256, `hub + 576`). Each marriage's child sits on row 2 at that marriage's edge midpoint: Asha at `(680+104)/2 = 392`, Priya at `(680+872)/2 = 776`, Rohan at `(680+1256)/2 = 968`. The middle child Priya's drop at x = 776 clears Alice's card (left edge 792) by 16px — the same `gap/2` margin the wing co-spouses get — so no child-drop crosses its co-spouse. Three thick marriage edges fan from Devraj's single bottom-midpoint; no marriage bars are rendered; the three half-siblings render in distinct sub-trees per P9.
...
### P9. Birth/adoption edges connect to the marriage edge
A child's birth or adoption edge attaches to the **midpoint of the marriage edge** of their parents' marriage — not to either parent card individually. This matches Kul's data model (`birth m_xxx` references the marriage id, not the parent ids). For monogamy that midpoint is the centre of the horizontal marriage-edge segment spanning the inter-card gap; the child's edge drops from there.

For polygamy hubs (P8 fan): each marriage's children hang from the **midpoint of that marriage's edge** — `children_center = (hub_cx + cospouse_cx)/2` — rather than from a segment between the spouses. Each child's birth/adoption edge originates at that midpoint (on the horizontal segment of the thick marriage edge, just below the hub) and fans down to the child. Because the children gather at the midpoint while the co-spouse mirrors to the wing, each marriage's children sit in their own column centred under their marriage edge, distinct from the half-siblings of every other marriage — e.g. in example 04 Priya hangs under the Devraj–Alice midpoint only; in example 15 (N=3) Asha under the Devraj–Meera midpoint, Priya under Devraj–Alice, Rohan under Devraj–Diana.
...
| **Marriage edge** | The unified marriage connector: a thick stroke (~8.75px in the default preview theme) connecting spouses, distinct from the thin birth / adoption edges (1.5px). **Monogamy** renders a horizontal segment between the two adjacent spouse cards, at the cards' vertical mid-height; the couple's children drop from its midpoint. **Polygamy** renders one edge per concurrent marriage of a hub (P8 fan, [ADR-0027](./adr/0027-fan-primitive-for-polygamy-hubs.md)), routed hub-card-bottom → co-spouse-card-top with the same orthogonal right-angle geometry as a birth edge (see the detailed row below). An **ended** (divorced) marriage renders translucent. (The DATA concept is still a `MarriageBar` in `kul-render`; only the rendered primitive changed from block to edge.) |
| **Adoption bar** | The join-point for an adoption sub-statement; same orthogonal-routing role the marriage edge plays for births. |
| **Birth edge** | Solid line. Routes within a tree (marriage-edge midpoint → child below) or across trees (canonical card → past or different-tree birth marriage). |
| **Adoption edge** | Dashed line. Same routing rules; edge style alone distinguishes from birth. |
| **Polygamy fan marriage edge** | The polygamy arm of the marriage edge above. One per concurrent marriage of a polygamy hub (P8 fan, [ADR-0027](./adr/0027-fan-primitive-for-polygamy-hubs.md)), routed from the hub card's bottom edge to the co-spouse card's top edge with the same orthogonal right-angle geometry as a birth edge. The co-spouse sits at the wing (mirror of the hub across the marriage's children-centre); the edge's horizontal segment runs at a bus just below the hub and its midpoint is the children-centre, where that marriage's child edges originate. The fan emerges from N marriage edges fanning out of the single hub-bottom point; there is no separate connector primitive. |
