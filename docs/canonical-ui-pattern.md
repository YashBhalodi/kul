...
*Known limitation (odd N):* with an odd number of marriages the middle co-spouse sits near the hub centre, so the clearance that lets a wing co-spouse's child-drop pass beside the co-spouse's card cannot hold for it — its marriage edge is near-vertical and its child hangs ~directly below the hub. This is accepted rather than fought (see ADR-0027).
...
*Worked example (`examples/15`):* N=3 polygamy. Devraj hosts Meera, Alice, and Diana, all ongoing, one child each (Asha, Priya, Rohan). Devraj's hub card sits alone on row 0 (centre 488). The three co-spouses splay across row 1 — Meera at the outer-left wing (104), Alice in the middle (488, coincident with the hub), Diana at the outer-right wing (872). Each marriage's child sits on row 2 at that marriage's edge midpoint: Asha at `(488+104)/2 = 296`, Priya at `(488+488)/2 = 488`, Rohan at `(488+872)/2 = 680`. The outer two marriage edges jog left (to Meera) and right (to Diana) with their children gathering toward the centre under each edge's midpoint; the middle marriage (Alice) exhibits the odd-N limitation — its edge is vertical and Priya hangs directly below the hub. Three thick marriage edges fan from Devraj's single bottom-midpoint; no marriage bars are rendered; the three half-siblings render in distinct sub-trees per P9.
