...
### Host and joining spouse

In a `marriage <id> <spouse_a> <spouse_b>` statement, `spouse_a` is the **host** and `spouse_b` **joins** the host's family. The host's canonical card stays in its host-lineage position; the joining spouse's canonical card sits adjacent to the host. Each host-lineage tree renders as an independent component, and a joining spouse's birth family is its own component too — connected to the host's marriage through the joining spouse's shared name identity plus a past-bio child-ghost in the bio family's children row (see *past intimacies emit ghosts*). The reader reconciles cross-family kinship through name pairing, not through spatial nesting.
...
*Worked example (`examples/05-cousins-and-in-laws`):* the Rossi and Bianchi lineages render as independent components placed left-to-right by source order on the shared generation grid. Marco Rossi's canonical card sits in the Rossi tree at his bio family's children row; his current marriage to Elena hosts her canonical card adjacent to him. Elena's birth into the Bianchi family is a past intimacy, so the Bianchi component carries a past-bio child-ghost for Elena in its children row — Elena's shared name on the canonical card and the ghost is what pairs the two ends for the reader. Later, Marco's son Matteo marries his cousin Giulia: both have canonical cards in the Rossi tree, Giulia at her own marriage to Matteo (chain step 1), so her bio family's children row carries a past-bio child-ghost where she would otherwise sit.

*Worked example (`examples/09-family-across-a-century`):* the Adeyemi, Okafor, and Eze lineages render as three independent components. Adunni's marriage to Chukwu places Chukwu's canonical card adjacent to Adunni in the Adeyemi tree, while the Okafor tree carries a past-bio child-ghost for Chukwu in its children row. Chukwu's mother Chiamaka, herself a joining spouse from the Eze line, is canonical in the Okafor tree at her marriage to Chukwu's father; the Eze tree carries a past-bio child-ghost for Chiamaka. Cross-family kinship reads through these ghost + canonical name pairs in source order, not through spatial nesting.
...
Every host-lineage tree renders as an independent component, placed left-to-right on the canvas by the source position of each component's first relevant declaration — a marriage if it has one, otherwise a person, otherwise the underlying marriage of a floating mini-component. Components mix freely in source order; there is no "orphans-last" bucket. The same rule applies inside a component: siblings (canonical and ghost alike) within a children row arrange in declaration order. Components share a global generation grid, so kin-symmetric persons in different components read on the same row. Author control is through declaration order — the same mechanism that controls the host rule.
