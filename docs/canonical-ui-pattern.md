...
*Worked example (`examples/12`):* Same polygamy hub as example 04 (Devraj concurrently married to Meera and Alice, Priya the bio child of `m_devraj_alice`), but Devraj is now a canonical child of Ramesh + Sita's marriage. The fan still applies, shifted down one generation: Ramesh + Sita's classical hub-and-flanks cluster sits on row 0 (monogamy, unchanged — its marriage bar is still rendered), Devraj's hub card sits alone on row 1 centred directly under the `m_ramesh_sita` bar (his birth edge enters his hub card from above), Meera and Alice splay to the wings on row 2, and Priya sits on row 3 at the Devraj–Alice midpoint. The whole fan nests as a single wide cluster under Ramesh + Sita without overlapping anything; the wing co-spouses are reserved in the hub's footprint so the components don't collide.
